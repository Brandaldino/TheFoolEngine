    ...
    void MainPass::Execute(RenderContext& ctx)
    {
        ...
        for (auto& output : m_Outputs)
        {
            ...
            if (ctx.RenderGraph->GetPointShadowMap(ctx.ShadowRenderer->GetPointShadowHandle()))
            {
                ...
                std::vector<float> farPlanes(MAX_SHADOW_LIGHTS, 100.0f);
                for (uint32_t i = 0; i < ctx.ShadowRenderer->GetPointShadowData().Count; ++i)
                    farPlanes[i] = ctx.ShadowRenderer->GetPointShadowData().Lights[i].FarPlane;
                m_Shader->SetFloatArray("u_PointShadowFarPlanes", farPlanes);
            }
            std::vector<glm::mat4> shadowLightsViewProjs;
            for (int i = 0; i < (int)ctx.ShadowRenderer->GetViewProjections().size() && i < MAX_SHADOW_LIGHTS; ++i)
                shadowLightsViewProjs.push_back(ctx.ShadowRenderer->GetViewProjections()[i]);
            m_Shader->SetMat4Array("u_ShadowMatrices", shadowLightsViewProjs);
            ...
            //    lightblock.Lights[i].ShadowIndex = ctx.Lights[i].ShadowIndex;
            //}
            //glNamedBufferSubData(ctx.ShadowRenderer->GetLightUBO(), 0, sizeof(LightGPUBlock), &lightblock);
