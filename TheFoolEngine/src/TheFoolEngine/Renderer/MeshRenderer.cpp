...
namespace TheFoolEngine
{
	...
	{
		static const uint32_t MaxVertexCount = 1000 * 100 * 2;
		static const uint32_t MaxIndexCount = 1000 * 100 * 6;
	...
	bool MeshRenderer::SetSLOTS(int slotPos, TextureType type, const Ref<Texture2D>& texture)
	{
		if (slotPos > 31 || slotPos < 12)
		{
			TF_CORE_ERROR("Batch Renderer Material SLOTS not applicable.");
			return false;
		}

		s_Data.SLOTS[slotPos] = type;
		s_Data.TextureSlots[slotPos] = texture;
